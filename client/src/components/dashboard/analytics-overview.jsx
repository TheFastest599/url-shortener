...
export function AnalyticsOverview({
	urls = [],
	selectedShortCode: initialShortCode,
}) {
	...
	const {
		...
		isRefetching,
		refetch,
	} = useAnalyticsOverview(selectedCode, {
		days,
		interval,
		includeBots: true,
	});
		...
		return (
			<Card className="border-border/70 bg-card p-12 text-center shadow-xs">
				...
				<p className="text-xs sm:text-sm text-muted-foreground max-w-sm mx-auto mt-1">
					Create your first short URL to start streaming real-time
					click telemetry and geographic analytics.
				</p>
			</Card>
		);
	}
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				...
				<Card className="border-border/70 bg-card shadow-xs">
					...
					<CardContent className="space-y-3">
						{isLoading ? (
							<Skeleton className="h-32 w-full" />
						) : (analytics?.topOperatingSystems || []).length >
						  0 ? (
